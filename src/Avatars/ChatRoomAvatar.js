...
export default class ChatRoomAvatar extends Component {
  ...
  render() {
    ...
    if (info.length === 1) {
      return <OneMemberRoomAvatar containerStyle={containerStyle} size={size} members={info}/>
    }
    if (info.length === 2) {
      return <TwoMembersRoomAvatar containerStyle={containerStyle} size={size} members={info}/>
    }
    if (info.length === 3) {
      return <ThreeMembersRoomAvatar containerStyle={containerStyle} size={size} members={info}/>
    }
    return <FourMembersRoomAvatar containerStyle={containerStyle} size={size} members={info.slice(0, 4)}/>
  }
}
